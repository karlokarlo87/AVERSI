...
function cleanPrice(priceText) {
    ...
    price = price.replace(/₾/g, '').replace(/ლ/g, '').replace(/ლარი/g, '');
...
// Safe retry helper
async function safeRetry(fn, context, maxRetries = 3) {
    let lastError;
    for (let i = 0; i < maxRetries; i++) {
        try {
            ...
            lastError = error;
            console.log(`⚠️ Retry ${i + 1}/${maxRetries} for ${context}: ${error.message}`);
            await delay(2000 * (i + 1));
        }
    }
    console.error(`❌ Failed after ${maxRetries} retries for ${context}`);
    return null;
}

// Safe goto helper
async function safeGoto(page, url, context) {
...
async function getscrapeSingleMatID(filename,matID) {
 
 
    try {
        console.log(`  🔍 Parsing MatID ${matID}...`);
        
       const html = fs.readFileSync(filename, 'utf-8');
       const $ = await cheerio.load(html);
         
      //  Check if product-summary exists
        ...
        const priceRaw  = $('.price del').text().trim() || '';
        const priceOldRaw= $('.price .amount.text-theme-colored').text().trim() || '';
        ...
        console.error(`  ✗ Error parsing MatID ${matID}:`, error.message);
        return null;
    }
...
// Download single MatID page HTML with Puppeteer
async function downloadMatIDPage(browser, matID) {
const allowedIDs = [
  90414, 36365, 77087, 138462, 9321, 28558, 79097, 999, 469, 15222,
  42353, 6669, 2807, 36357, 36356, 292, 125745, 86380, 80616,
  82920, 135757, 134837, 4628, 29687, 22836,148602,50631,80568,72737
...
let url ;
if (allowedIDs.includes(matID)) {
    console.log("allowedIDs.includes(matID)");
   // url = `https://www.aversi.ge/ka/aversi/act/drugDet/?MatID=${matID}`;
} else {
    console.log("!allowedIDs.includes(matID)");
   // url = `https://shop.aversi.ge/?dispatch=aversi.redirect&matid=${matID}`;
}
//url = `https://www.aversi.ge/ka/aversi/act/drugDet/?MatID=${matID}`;
     url = `https://shop.aversi.ge/?dispatch=aversi.redirect&matid=${matID}`;
    let page;
    ...
    try {
        ...
        console.log(`  Downloading MatID ${matID}...`);
        ...
        const filename = path.join(tempDir, `matid_${matID}.html`);
        ...
        console.error(`  ✗ Error downloading MatID ${matID}:`, error.message);
        if (page) await page.close();
        return null;
    }
}

// Parse MatID HTML file with Cheerio
async function parseMatIDFile(filename, matID) {
    try {
        console.log(`  🔍 Parsing MatID ${matID}...`);
...
// Scrape all MatID products (refactored to download then parse)
async function getRandomNumber(browser) {   
    const numbers = [
        ...
        36919, 5724, 15945, 28072, 28137, 77087, 138462, 26991, 9321,  128831,28558,
        ...
        15222, 89049, 71587, 76882, 84113, 66145, 85390, 32261, 796, 42353,
        ...
        3718, 8356, 4628,71104, 135757, 11267, 134837,  126930, 80636,
        ...
        97179, 86381, 97180, 37399, 78125, 29687, 74768, 72737, 143132, 22836
    ];
...
const allowedIDs = [
  ...
  82920, 135757, 134837, 4628, 29687, 22836,148602,50631,80568,72737
    ...
    // 13118,3479,35667,8310,7370,8299,8201,26690,40230,42201,11410,35281,8177,5074,
    // 37906,8146,8146,39356,3179,3164,3172,8091,12701,3114,7827,21493,38560,7709,7744,
    // 7743,27293,7662,7660,7170,36266,25212,25214,2889,6076,25593,12149,47538,2764,27143,
    // 7534,25804,24864
    console.log(`\n🔍 Starting MatID scraping (${numbers.length} products)...`);
    console.log('📥 Method: Download HTML → Parse with Cheerio\n');
    //90414,36365,77087,138462,9321,28558,79097,999,469,15222,42353,6669,2807,36357,36356,292,125745,86380,80616,82920,135757,134837,4628,29687,22836
    const allProducts = [];
    let successCount = 0;
    let failCount = 0;
    
    for (let i = 0; i < numbers.length; i++) {
        const matid = numbers[i];
        console.log(`\n[${i + 1}/${numbers.length}] MatID: ${matid}`);
        
        // Step 1: Download HTML page
        const filename = await downloadMatIDPage(browser, matid);
 ...
 let product;
if (allowedIDs.includes(matid)) {
      product = await getscrapeSingleMatID(filename, matid);
} else {
      product = await parseMatIDFile(filename, matid);
}
 //product = await getscrapeSingleMatID(filename, matid);
            // Step 2: Parse HTML with Cheerio
          //  const product = await parseMatIDFile(filename, matid);
            ...
            if (product) {
                ...
                failCount++;
                console.log(`  ❌ Failed to extract product (${failCount}/${i + 1})`);
            }
            ...
            failCount++;
            console.log(`  ❌ Failed to download (${failCount}/${i + 1})`);
        }
    ...
    console.log(`\n✅ MatID scraping complete!`);
    console.log(`   Success: ${successCount}`);
    console.log(`   Failed: ${failCount}`);
    console.log(`   Total: ${allProducts.length} products added\n`);
...
async function scrapeAllCategories(browser, categories) {
    ...
    console.log('🔍 Starting MatID scraping (220+ products)...');
